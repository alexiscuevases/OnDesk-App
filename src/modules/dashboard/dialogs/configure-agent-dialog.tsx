...
import { useEndpoints } from '@/modules/endpoints/hooks/use-endpoints';
import { toast } from "sonner";
import { Agent, UpdateAgentInput, updateAgentSchema } from '@/modules/agents/lib/validations';
import { ManageEndpointsDialog } from "./manage-endpoints-dialog";
import { AGENT_MODELS_OBJECT, AGENT_STATUSES_OBJECT, AGENT_TYPES_OBJECT } from '@/modules/agents/lib/constants';
												...
												{endpoints.slice(0, 3).map((endpoint) => (
													<div key={endpoint.id} className="flex items-center justify-between p-2 rounded-md bg-muted">
														<div className="flex items-center gap-2">
															<Badge
																variant="outline"
																className={`text-xs ${endpoint.method === "GET"
